 ...
 */
class BarHighlighter(chart: BarDataProvider) : ChartHighlighter<BarDataProvider>(chart) {
    ...
    override fun getHighlight(x: Float, y: Float): Highlight? {
        ...
        val set = barData.getDataSetByIndex(high.dataSetIndex) as? IBarDataSet
        if (set?.isStacked == true) {
            return getStackedHighlight(high, set, pos.x.toFloat(), pos.y.toFloat())
        }
    ...
    fun getStackedHighlight(high: Highlight, set: IBarDataSet, xVal: Float, yVal: Float): Highlight? {
        ...
        // not stacked
        val yVals = entry.yVals ?: return high
